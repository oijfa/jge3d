	...
	public boolean handleMouse(Event evt) {
		...
		case 1:
			if (evt.getType() == Event.Type.MOUSE_BTNDOWN) {
				 ...
				 * ent.setPosition(camera.getPosition()); Parser parser = new
				 * XGL_Parser(); try{
				 ...
				 * //parser.
				 * readFile("resources/models/misc/0335-CATHODE_ASSEMBLY.obj");
				 * }catch(Exception e){ e.printStackTrace();
				 * System.out.println("Model loading failed"); }
				 * ent.setModel(parser.createModel()); Vector3f impulse =
				 * camera.getRayTo(Mouse.getEventX(), Mouse.getEventY());
				 * impulse.scale(0.02f); ent.setGravity(new Vector3f(0,0,0));
