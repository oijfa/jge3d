...
public class ButtonRunnable implements Runnable {
	...
	public ButtonRunnable(RotationMenu objToUse, String buttonName,
		ButtonModel buttonModel) throws SecurityException,
	...
	public void run() {
		if (buttonModel.isArmed() && buttonModel.isPressed()) {
			try {
				...
				System.out.println("Couldn't start inputrunnable - "
					+ buttonName);
				e.printStackTrace();
			}
			...
			try {
				...
				System.out.println("Couldn't stop inputrunnable - "
					+ buttonName);
				e.printStackTrace();
			}
		}
