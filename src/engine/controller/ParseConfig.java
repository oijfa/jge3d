...
public class ParseConfig {
	...
	public void readConfigFile(String path) throws Exception{
		...
		String configName;
		engine.window.tree.Model treeModel = new engine.window.tree.Model();
	...
	private void createItem(Element ele, TreeTableNode parent, String configName, HashMap<String, Vector3f> defaultPositions) throws Exception {
		...
		if(show == true){
			engine.window.tree.Node item;
			if( parent.getClass() == engine.window.tree.Model.class ){
				if(color != null){
					item = ((engine.window.tree.Model)parent).insert(new ColoredTextString(name, color.get(0), color.get(1), color.get(2)), "");
				}else{
					item = ((engine.window.tree.Model)parent).insert(name, "");
				}
			}else{
				if(color != null){
					item = ((engine.window.tree.Node)parent).insert(new ColoredTextString(name, color.get(0), color.get(1), color.get(2)),value);
				}else{
					item = ((engine.window.tree.Node)parent).insert(name,value);
				}
			}
