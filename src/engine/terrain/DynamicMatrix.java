...
public class DynamicMatrix {
	...
	public void addNoise() {
		...
		for(int i=0;i<(columns*rows);i++) {
			...
			current.add(randomizeNoBreaks(indexToRowCol(i)[0],indexToRowCol(i)[1]));
			table.set(i,current);
		}
	}
	
	public Vector3f randomVector(int column, int row) {
		//We don't want to randomize the x and z as that would make the terrain overlap
	...
	public Vector3f randomizeNoBreaks(int column, int row) {
		...
		//terrain
		if(found_left && found_up)
			y=(up+left)/2;
		else if(!found_left && found_up)
			y=up+randomInt();
		else if(found_left && !found_up)
			y=left+randomInt();
		else
