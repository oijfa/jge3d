...
public class Model<E extends Number> {
	...
	public void draw() {
		...
		/*
		 * GL11.glBegin(GL11.GL_LINES); GL11.glVertex2i(-5, 0);
		 * GL11.glVertex2i(5, 0); GL11.glVertex2i(0, 5); GL11.glVertex2i(0, -5);
		 * GL11.glEnd();
		 */
	}
	...
	public void increaseScale(int steps) {
		if (steps > 0) {
			CubicGrid<Block<E>> new_grid = new CubicGrid<Block<E>>(
				(int) (size + (size * Math.pow(2, steps))));
			int x_corner;
			int y_corner;
			int z_corner;
			for (int z = 0; z < size; z++) {
				for (int y = 0; y < size; y++) {
					for (int x = 0; x < size; x++) {
						...
						z_corner = z * size;
						// Set center
						new_grid.set(x_corner, y_corner, z_corner,
							cubic_grid.get(x, y, z));
				...
				for (int y = 0; y < new_size; y++) {
					for (int x = 0; x < new_size; x++) {
						...
						new_grid.set(
							...
							y,
							z,
							averageCube(cubic_grid, x_corner, y_corner,
								z_corner, steps));
					}
				}
			}
	...
	private Block<E> averageCube(CubicGrid<Block<E>> cubic_grid, int x_center,
		int y_center, int z_center, int steps) {
